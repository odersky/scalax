...
class SparseArray[Value: ClassTag]:
  import SparseArray._

  private var siz: Int = 0
  private var root: Node[Value] = LeafNode()

  private def grow() =
    val newRoot = InnerNode[Value](root.level + 1)
   ...
   */
  def transform(op: Transform[Value]): Unit =
    root.transform(op, 0)
  ...
  override def toString =
    val b = StringBuilder() ++= "SparseArray("
    var first = true
...
object SparseArray:
  ...
  trait Transform[Value]:
    def apply(key: Int, v: Value): Value

  private[util] inline val NodeSizeLog = 5
  private[util] inline val NodeSize = 1 << NodeSizeLog
  private[util] inline val MaxLevels = 5  // max size is 2 ^ ((MaxLevels + 1) * NodeSizeLog) = 2 ^ 30
  ...
  abstract class Node[Value](val level: Int) extends Repr:
    protected[SparseArray] def elemShift = level * NodeSizeLog
    protected[SparseArray] def elemSize = 1 << elemShift
    protected[SparseArray] def elemMask = elemSize - 1
    def contains(index: Int): Boolean
    def apply(index: Int): Value
    ...
    def transform(op: Transform[Value], offset: Int): Unit
    def nodeCount: Int
  end Node

  class LeafNode[Value: ClassTag] extends Node[Value](0):
  ...
  private class InnerNode[Value: ClassTag](level: Int) extends Node[Value](level):
    private[SparseArray] val elems = new Array[Node[Value]](NodeSize)
  ...
  private def newNode[Value: ClassTag](level: Int): Node[Value] =
